...
$\left\{ x \right\}$ is the unit set of $x$, and $\left\{ x,y \right\}$ is the pair set of $x$ and $y$
...
$$
are the same set.
We can also describe a set using the notation:
$$
...
Two sets $R$ and $S$ are equal if $(\forall x)(x\in A \iff x\in B)$, which is the same as saying if [[Subsets|$R\subseteq S$]] and $S\subseteq R$
