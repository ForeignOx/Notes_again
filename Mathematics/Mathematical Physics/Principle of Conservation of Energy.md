...
## In the Hamiltonian Sense
We can apply the [[Hamiltonian|Hamiltonian equations]] for a function on [[Phase Spaces|phase space]] generated by the [[Hamiltonian Flow|hamiltonian flow]] to show the coservation of energy
In the hamiltonian formalism, energy is equal to the hamiltonian itself. So the equation for energy conervation can be written as
$$
\frac{d H}{dt} =\frac{ \partial H }{ \partial t } +\left\{ H ,H\right\}=\frac{ \partial H }{ \partial t } 
$$
Using the fat that the [[Poisson Bracket|poisson bracket]] is antisymmetric
We can see that if time does not apprear explicitly in the expression for the Hamiltonian, then energhy i conserved
