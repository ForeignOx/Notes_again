...
As this seemed insufficient, a more specific approach was taken, giving more thought to the geometry of the protein. For this the data given by the matrix generated by the values of $K_{ab}^{(k)}$, representing the interaction between edge $a$ and $b$ at decomposition level $k$. For any given edge $$