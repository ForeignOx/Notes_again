...
For an element $a$ of a finite group $(G,*)$ such that $a$ has order $k$, the cyclic [[Subgroups|subgroup]] generated by $a$ is $<a>:=\{ e,a,a^{2},\dots,a^{k-1} \}$, if $<a> = G$ then we call the group itself [[Cyclic Groups|cyclic]] and state that it is generated by $a$
